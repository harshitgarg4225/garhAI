...
| **Still never run (or red)** | `docker compose up` verbatim (Railway proves the images, not the compose wiring). A fully green GitHub Actions run — the first two runs ever (2026-08-27) executed and went red on exactly two environment-only findings (a prettier drift in this file, a `setuptools` licence-metadata gap on CI's fresh pip), both fixed; ruff/eslint/format passed IN CI. The downstream stages (typecheck, units, golden, e2e-smoke) have still never executed on a runner. The real Anthropic/Stability providers (adapters built and unit-tested; keys are a launch gate). The visual-regression suite (needs a CI-minted baseline). mypy debt outside the clean trees (garh_api 56, garh_rules 44, solver 79, render 28, drawings 801 — counts in ci.yml). |
...
1. **CI fired for the first time on 2026-08-27 — and is not fully green yet.**
   The lint stage passed on a real runner (ruff 0, eslint 0, after burning
   ~2,638 + ~540 errors); the first runs' two failures were environment-only
   (prettier drift, `setuptools` licence metadata) and are fixed. The stages
   behind them — typecheck, units, golden, and especially the e2e-smoke job's
   service containers — have never executed on a runner. Trust the Actions
   tab, not this sentence.
