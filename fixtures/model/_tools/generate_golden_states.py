...
def cases() -> List[Dict[str, Any]]:
    ...
    # ---- transform-produced logs (copy / paste / array / mirror) ----------
    #
    # `garh_model.transform` invents NO op type: a paste is a group of wall.add /
    # opening.add / room.assign, a mirror in place is wall.delete + wall.add. So
    # the planner's output is an ordinary op log and belongs HERE, in the fold's
    # own cross-language corpus, on top of the op-level contract that
    # golden-transforms.json pins for the planners themselves. These rows are the
    # ones that would catch a fold — rather than a planner — treating a
    # transform's log differently on the two sides.
    #
    # The logs are generated by running the real planner, so they cannot drift
    # from what the product actually dispatches.
    walls = [
    ...
    def _plan_ops(base: List[Dict[str, Any]], make_plan: Any) -> List[Dict[str, Any]]:
        doc = apply_group(empty_project_doc("ft-in"), [Op.from_json(o) for o in base]).model
        result = make_plan(doc)
        if not result.ok or result.plan is None:
            raise SystemExit(f"transform golden refused: {result.refusal}")
        return base + [o.to_json() for o in result.plan.ops]

    paste_base = _two_room_ops() + _opening_ops() + [
        _op("storey.add", id=FF, index=1, name="First Floor", heightMm=3000)
    ]
    out.append(
        {
            "name": "transform-paste-floor-up",
            "description": (
                "The op log a PASTE dispatches: five walls and their two openings copied onto a "
                "new first floor with a zero delta. No new op type — wall.add and opening.add, "
                "with ids derived from the gesture's group id so the log is reproducible. Applied "
                "as one group, this is one undo."
            ),
            "unitsDisplay": "ft-in",
            "ops": _plan_ops(
                paste_base,
                lambda doc: plan_paste(
                    doc,
                    PasteRequest(
                        element_ids=walls,
                        group_id=fixed_id("group", "GSPASTE"),
                        delta_mm=Pt(0, 0),
                        target_storey_id=FF,
                    ),
                ),
    ...
    out.append(
        {
            "name": "transform-mirror-copy",
            "description": (
                "The op log a MIRROR (keep original) dispatches: the same five walls reflected "
                "about x = 9000 onto clear ground. Every 'in-*' swing becomes 'out-*' with the "
                "hinge token unchanged and offsetMm untouched — a reflection is an isometry along "
                "the wall and orientation-reversing across it."
            ),
            "unitsDisplay": "ft-in",
            "ops": _plan_ops(
                _two_room_ops() + _opening_ops(),
                lambda doc: plan_mirror(
                    doc,
                    MirrorRequest(
                        element_ids=walls,
                        group_id=fixed_id("group", "GSMIRROR"),
                        axis="vertical",
                        at_mm=9000,
                    ),
                ),
    ...
    out.append(
        {
            "name": "transform-mirror-in-place",
            "description": (
                "The op log a MIRROR with 'keep original' OFF dispatches — flipping the plan. It "
                "deletes every selected wall before re-adding any of them, at the ORIGINAL ids, "
                "because south and north swap positions and wall.move would trip WALL_DUPLICATE "
                "whichever went first. The openings the delete cascade took come back with the "
                "hand flipped."
            ),
            "unitsDisplay": "ft-in",
            "ops": _plan_ops(
                ...
                lambda doc: plan_mirror(
                    ...
                    MirrorRequest(
                        element_ids=walls,
                        group_id=fixed_id("group", "GSFLIP"),
                        axis="horizontal",
                        keep_original=False,
                    ),
                ),
