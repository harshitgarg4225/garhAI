...
| Billing                                                                                     | 🟡 mock provider by default; `PROVIDER_BILLING` is the switch (`billing_live` in DEFAULT_FLAGS is read by nothing); Razorpay adapter built, never run live — see docs/billing-verification.md |
