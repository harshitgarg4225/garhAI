...
0/10 means nothing is done.

**The scores in the table have been moved since**, where a gap the reader named is closed
and the closing is executed rather than argued; every such move carries a "since the
reading" note saying what landed and what is still open. Nothing here is at 10, and that
is the honest answer to the bar: 10 means the minutest details, and each of these jobs
still names something real. The four raised on 2026-09-20 (J01, J04, J07, and the earlier
wave's J02/J05/J06/J14) sit at 9 because each has one substantive item left, written into
its row. The evidence
...
| Job                                   | Score | Verdict                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------- | ----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| J01 Sign in and set up the practice   |     9 | Every blocking gap the reader listed is closed and re-read on 2026-09-20: invites, team roles and seat accounting, Practice/Team/Account settings, 2FA enrol-activate-disable and the login screen's two-factor challenge, signed-in devices, and the login page's error branches matching the server. The mobile field that was collected and discarded is gone. The join itself now runs in a browser on every push (`team.spec.ts`: admin invites → colleague signs in from a separate context → they are an editor in the same firm), with the control that an address nobody invited cannot sign in at all. Open: 2FA has no browser run of its own, and the seeded Brevo path is proven by a single live sign-up rather than a suite.   |
| J02 Capture the plot                  |     9 | Deed entry, deed-area/diagonal readouts, geometric edge roles shared by engine and editor, distinct side-A/side-B setbacks, DXF survey import that counts every skipped entity, and the scanned survey as a traceable layer — every handle proven by real pointer events in a browser. Open: PDF underlays, and the @plot spec is not in a CI grep list.                                                                                                                                                                                                                                                                                                                                                                                      |
| J03 Capture the client brief          |     5 | The main path — typed form or pasted text → assumption chips → one undoable brief                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| J04 Generate compliant plan options   |     9 | Options carry their seed and a plain-prose rationale, the zero-options screen shows the worker's diagnosis, and parking is measured off the plan, placed by the solver and drawn on A-01 — one catalogue entry for all three. The §14 time budget and determinism profile run against real CP-SAT. Open: coverage is 37/60 against the packs' real setbacks. The other two — the solver gate ignoring accepted overrides, and nothing clicking Generate in a browser — closed on 2026-09-20 (`gates.blocking_rule_failures`; browser UAT runs 14–16, which press Generate and get options back from the real solver).                                                                                                                         |
| J05 Edit the plan in 2D               |     9 | Wall split and column tools, storey removal, copy/paste/duplicate/mirror/array, trackpad and stylus handling, skew-wall dimension chains with perpendicular and intersection snaps, and hatches at the sheet's own density — with the @canvas suite and the §14 budgets now running in CI. Open: per-tool pixel assertions for drag handles, opening slide and stair rotate; hatch is tied to the sheet's constant, not diffed against the sheet.                                                                                                                                                                                                                                                                                             |
...
| J07 Check compliance continuously     |     9 | The engine, the live re-check, "Fix it", overrides with a reason, the numbers an architect needs (measured vs limit, per-element instances, severity, citations, area statement, Vastu score) and a named failed re-check are all executed; door reachability is now a rule on the tab rather than a solver-only gate, negative-tested both ways. Open: the shaft story is a stub end to end — `hasShaftAccess` is parsed into the context, serialised back out, and read by NO rule; no rule gates on `roomIsInternal` either, so an internal wet area is checked exactly like an external one, and the packs mention a shaft only inside a `fix` hint string (task #48). And all 120 values are unreviewed seeds (a launch gate, not code). |
...
| J14 Trust and operations              |     9 | Worker heartbeats, an owner ops page, migrations under an advisory lock, scheduled backups with a rehearsal that refolds real op logs to the recorded state hash, Railway config-as-code held to the runbook, a product-path load smoke, the onboarding tour, Settings → Privacy with the audit trail and DPDP rights, and an axe pass that found two serious violations. Open: @a11y is not in CI (axe-core is not in the lockfile), and the owner must still set the variables in the runbook.                                                                                                                                                                                                                                              |
...
**Score 9** (was 5.5 at the reading). Sign-up → OTP → session → sign-out is executed, hardened and green (280 API tests, CI browser smoke, one live Brevo sign-up on the deployed stack), but "set up the practice" stops at a firm name: no invite/team/roles/account-settings UI or endpoints exist, seats and 2FA/devices are backend-only, and the login form collects a mobile number it silently discards.
...
Since the reading: A reload that raced a token refresh no longer signs the architect out (30-second reuse leeway on refresh rotation, family still revoked on real theft). Merged 2026-09-09: an admin invites a colleague by email (token stored hashed, a pre-auth status page, acceptance is the ordinary sign-in, editor invites count against the plan's seats, viewers are free, anti-enumeration preserved with a resend cooldown of its own), runs the team (role change never the last admin, removal releases the seat and ends every live session), edits the practice (name, address, GSTIN with check digit, registration, phone, and the rename reaches the sheet title block) and their own name and CoA number; Settings has Practice, Team and Account sections and the login page matches the server. Migration 0015. 375 API tests across the team, auth, tenancy and audit suites on the merged tree.

Since the reading — wave 3 (2026-09-20): every gap above was re-read against the tree and
each is closed in code: the login form no longer collects a mobile number it throws away,
`AccountSection` enrols/activates/disables a second factor and lists signed-in devices,
`LoginPage` handles `two_factor_required` and branches on the server's real `otp_invalid`,
and `LoginPage.test.tsx` plus `session.test.ts` cover the countdown, resend and sign-out
paths. What was still unexecuted was the JOIN — the thing no unit test can assert, because
every fixture already agrees who is in which firm. `e2e/tests/team.spec.ts` now walks it in
a real browser across two contexts: the admin invites from Settings → Team, the colleague
opens the emailed link in a context of their own, signs in, and lands inside the SAME
practice as an editor, with the open invite gone and the invite form not theirs to use. Its
control turned out stronger than intended: an address nobody has invited cannot sign in at
all — the screen still says "Check your email" (§13 non-enumerability) but no code is issued,
so the link is demonstrably what let the colleague in. Runs in CI on every push
(`pnpm --filter @garh/e2e test:team`).
...
Since the reading — wave 2 (2026-09-20), merged: a plot is entered from the sale deed, the
editor states deed area, every diagonal and the perimeter, geometric edge roles are computed
once and shared by the engine, the panel and the 2D envelope, side A and side B carry distinct
setbacks, `PUT /plot` keeps road names, and a DXF survey of LINE/ARC entities chains into a
boundary that names every entity it had to skip. The scanned survey can be traced over in the
plot editor, reusing the Plan tab's underlay record rather than a second calibration.

Two defects that only running it could find: the shared length input committed on ANY blur and
re-parsed text it had itself rounded to the inch, so tabbing through a deed side moved it 11 mm;
and a browser negative control passed against a stale bundle until Vite was restarted with
`--force`, which is worth knowing before trusting any local click test here.
...
Since the reading — wave 2 (2026-09-20), merged: every option carries the seed that produced it
and a rationale in prose with the numbers behind it, and a run that yields nothing shows the
worker's own diagnosis instead of a blank screen. Car parking stopped being a declaration: the
rule counts bays measured off the model, the solver places them in the front setback, and A-01
draws them — all three sized from one catalogue entry, which a test holds together under a
`GARH_CATALOG_DIR` override. The §14 time budget and the determinism profile execute against
real CP-SAT, and any orthogonal plot is described with an honest statement of what skew costs.

The coverage sweep's own caveat was backwards. It warned its fixed 1.5 m front setback judged
small plots harshly; Bengaluru's road-width band is 3 m and overrides the plot band, so every
row had been flattered. Against the packs' real setbacks it is 37/60, not 40/60: twelve of the
failures are the whole 20 × 30 column, which is a rule-pack question for empanelled review, and
eight are G+0 briefs that all solve at G+1.
...
Since the reading — wave 2 (2026-09-20), merged: wall split and column tools, storey removal,
copy/paste/duplicate/mirror/array, trackpad two-finger pan versus pinch with touch and stylus
handled, aligned dimension chains for skew walls with perpendicular and intersection snaps, and
hatches on the plan at the drawings service's own spacing — read from that service's constant
rather than a forked table, with a control that rewrites the constant and proves the gate goes red.

Two findings outlast the features. A TypeScript parameter property in the model package was the
one construct Node's strip-only transform cannot load, so every browser spec importing the model
listed ZERO tests: the canvas DoD walk had been unloadable for weeks while typecheck, lint and
vitest all stayed green. And removing a layer's PickRegistry registration left its click test
green in a real browser, twice, because the select tool falls back to a geometric search when the
raycast is empty — every click-based pick assertion in this repo shared that blind spot, and the
specs now ask the raycast directly before they click.
...
Since the reading — wave 3 (2026-09-20): door reachability is a COMPLIANCE RULE now, not
only a solver gate. `garh_model.circulation` walked the door graph for solver candidates
and library plans and nothing else, so a plan an architect drew, imported from DXF or
edited after generating could still hide a room nobody can walk into while the tab read
0 fail — CLAUDE.md bug 7, fixed for exactly one producer of plans. `nbc.circulation.room.reachable`
(fail) and `nbc.circulation.room.not_through_bath` (warn) read the walk's verdict through a new
`room_access` check; the engine still does no pathfinding, `build_evaluation_context` carries
`room.access`, and a document whose connectivity was never derived is `not_applicable` with
`roomAccessKnown` named rather than a pass. Both halves negative-tested
(`test_compliance_reachability.py`, 4 cases): stop the projection carrying `access` and three go
red, remove the gate and the null case reports `fail` over a model nobody measured. Every
ready-made plan passes the new rules unchanged — the first independent evidence that the
solver's gate and the model's geometry agree about doors. 120 rules now. The `access` unit reads
"No door route reaches it" on the tab rather than "unreachable of reachable, only-via-bath".

Also wave 3: an accepted override now un-blocks Generate, which the override route's own
docstring had always claimed — `gates.blocking_rule_failures` skips a `fail` row the
architect accepted, and ONLY that kind, because the row JSON sets `overridden: true` for a
value override too and a design that fails against the architect's own limit has been
accepted by nobody. Negative-tested in both directions, plus a cross-boundary case that
posts a real override through HTTP and puts the row the ROUTE produced in front of the
gate. And the A-06 annexure now prints the accepted deviations as its FIRST footnote:
`AreaStatement.overridden_rule_ids` had carried them since Phase 2 and its own docstring
said the annexure marked them, which it did not — so a submission set could show a FAR or
setback that fails, correctly measured, with nothing on the paper saying the deviation was
deliberate and recorded. The control is that a clean set stays silent.

And "Fix it" has been pressed. `plan-canvas.spec.ts` drives the whole journey in a
browser: a 750 mm front door against the pack's 900 mm minimum, press Fix it, the door
reaches 900 on the server, the row turns `pass`, the button withdraws; press Undo in the
top bar and all three come back. Two things fell out of the first run. The row's
"FAILS"/"ADVISORY" badge is the rule's SEVERITY, not its status, so it reads "FAILS" on
a rule that passed — the row now carries `data-status` because only the chip's colour
said otherwise. And "Fix it" was offered on PASSING rules: `fixAvailable` from the
server means "the pack declares a fix", a property of the rule rather than of the plan,
so one click on a compliant 1 000 mm door would have resized it DOWN to the 900 mm
minimum. The view-model now gates the offer on the rule actually being broken
(`fail` or `warn`), with the two other conditions kept as the negative control.

Since the reading: Merged 2026-09-09: "Fix it" applies a real op group for the strategies a client can compute honestly (storey height, opening width, stair riser, projection) and shows no button for the rest; overrides are accepted with a reason, stamped with who and when, revocable, audited (`compliance.overridden`, `compliance.override_revoked`) and carried into the area statement; the tab shows measured against limit in project units, per-element instances, severity and Hard badges, citations with review standing, the area statement, the Vastu score, engine warnings, filters, group-by-storey and a print annexure; a failed re-check is named instead of a stale green strip; a stale frozen report is no longer served as current after an edit; `hard` means "cannot be relaxed", the solver gate rejects any failing row and the export check honours accepted overrides (task #46 closed on the API side). Migration 0016. The three items listed here as open on 2026-09-09 — the browser run of Fix-it, the annexure printing accepted deviations, and the solver gate honouring acknowledgements — all closed on 2026-09-20; see the wave 3 note above.
...
Since the reading: CI is green again after two formatting misses; the production connections were read from the deployed stack's boot lines and recorded (Anthropic, Stability and Brevo live; Razorpay and Sentry absent; APP_ENV=dev on the api). Partial merge 2026-09-16: every worker writes an expiring heartbeat; GET /admin/ops reports queues, workers, job percentiles, providers, Sentry state and the migration head, with an owner page at /platform/ops that names no secret; migrations take a Postgres advisory lock and the boot seed writes nothing twice; scheduled backups to the bucket with a rehearsal that checks the data; Railway config-as-code per service with a test that holds it to the runbook. Still in flight: the product-path load test, the onboarding tour, the Security and Privacy settings surfaces, the accessibility pass and the go-live checklist. Two findings from merging that work, both fixed: the §13 tenancy audit flagged the new migrate module for running SQL outside the repository layer (the advisory-lock helpers now live in the connection-plumbing module, which is the one place the audit allows it), and the render-jobs suite passed on a bare database only when the billing tests had run first, because the quota gate reads a billing table that lives on its own metadata — the shared test fixture now creates it, and the suite ran 7 failed then 15 passed on the same database.

Since the reading — wave 2 (2026-09-20), merged and complete: the onboarding tour, Settings →
Privacy with the audit trail and the DPDP rights an architect can actually reach, and an
accessibility pass over every screen. The axe run found two serious violations, one of them in
the shared chip primitive — `opacity-70` over 11 px uppercase, 3.42:1, on every Brief page since
the chip was written, invisible to the unit suite because jsdom computes no styles.

The tour then caused the wave's clearest regression, and CI caught it: its dim layer intercepted
every click, so the core smoke journey retried the plot button 33 times and timed out, and the
browser UAT found the other half — opening the plan tab auto-started the tour on its Brief step
and the route followed. The overlay now dims without taking pointer events, and auto-start opens
the step belonging to the tab already on screen. A first-time architect is informed, never blocked
