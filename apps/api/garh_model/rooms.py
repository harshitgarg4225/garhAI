...
def build_half_edge_graph(walls: Sequence[WallLike]) -> HalfEdgeGraph:
    ...
    for u, v, wall_id, thickness_mm in edge_by_key.values():
        ...
        id_b = id_a + 1
        half_edges.append(
            HalfEdge(id=id_a, from_node=u, to_node=v, twin=id_b, wall_id=wall_id, thickness_mm=thickness_mm)
        )
        half_edges.append(
            HalfEdge(id=id_b, from_node=v, to_node=u, twin=id_a, wall_id=wall_id, thickness_mm=thickness_mm)
        )
    ...
    for n in range(len(outgoing)):
        ...
        def compare(x: int, y: int, origin: Pt = origin) -> int:
            c = compare_angle_around(origin, nodes[half_edges[x].to_node], nodes[half_edges[y].to_node])
...
class RoomCandidatesResult:
    ...
    outline: Optional[Tuple[Pt, ...]]
    non_integral_crossings: int


def room_candidates(
    walls: Sequence[WallLike],
    min_room_area_mm2: Optional[int] = None,
) -> RoomCandidatesResult:
    ...
    outer_face: Optional[PlanarFace] = None
    ...
    outline: Optional[Tuple[Pt, ...]] = None
    if outer_face is not None:
        entries = [_RingEntry(pt=p, dist=outer_face.inset_mm[i]) for i, p in enumerate(outer_face.ring)]
        simplified = _merge_collinear(_drop_spurs(entries))
        if len(simplified) >= 3:
...
def match_rooms(
    ...
    for ci, cand in enumerate(candidates):
        ...
        for ei, room in enumerate(existing):
            ...
            if cb.max_x < rb.min_x or rb.max_x < cb.min_x or cb.max_y < rb.min_y or rb.max_y < cb.min_y:
                continue
