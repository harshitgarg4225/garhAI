...
_Last audited 2026-08-21, at the close of Phases 6 + 7._
...
Read every command below as _intended_ behaviour, and read the verification
...
5. **Four audited actions have no write site yet.** `compliance.overridden`,
   ...
   the phase that fills each, and the test fails if a _new_ action goes un-emitted
...
Docker alone is enough to _run_ the stack — Node and Python are only needed for
